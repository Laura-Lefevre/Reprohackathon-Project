...
### Description : R file for functionnal           ###
### enrichment                                     ###
######################################################
...
###########################################################
# LOAD OFFLINE DATA (now via explicit paths)
###########################################################
gene_ids_df      <- read.csv(gene_ids_file, stringsAsFactors = FALSE)
gene_pathways_df <- read.csv(gene_pathways_file, stringsAsFactors = FALSE)
pathways_list    <- readRDS(kegg_file)

###########################################################
# WRITE FILTERED IDS
###########################################################
sub_gene_ids <- gene_ids_df %>% filter(locus_tag %in% clean_tags)
write.csv(sub_gene_ids, file.path(outdir, "gene_ids.csv"), row.names = FALSE)

###########################################################
# WRITE FILTERED PATHWAYS
###########################################################
sub_gene_pathways <- gene_pathways_df %>% filter(locus_tag %in% clean_tags)
write.csv(sub_gene_pathways, file.path(outdir, "gene_pathways.csv"), row.names = FALSE)
...
write.csv(annot_long, file.path(outdir, "annot_long.csv"), row.names = FALSE)
...
write.csv(df_enrich, file.path(outdir, "df_enrich.csv"), row.names = FALSE)

df_top25 <- df_enrich %>% arrange(desc(odds_ratio)) %>% head(25)
write.csv(df_top25, file.path(outdir, "df_enrich_top25.csv"), row.names = FALSE)
